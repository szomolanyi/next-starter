      ...
      if (typeof window === 'undefined') {
        ...
        if (ssr) {
          try {
            ...
            console.log('******** run getDataFromTree 8888 ');
...
function createApolloClient(initialState = {}, { getToken }) {
  ...
  */
  ...
  console.log(`create httplink uri=${process.env.GRAPHQL_URI}`);
  ...
  const authLink = setContext((request, { headers }) => {
    ...
    return {
      headers: {
        ...headers,
        authorization: token ? `Bearer ${token}` : '', // TODO toto server ignoruje
        Cookie: token || '',
      },
    }
  })
...
function getToken(req) {
  ...
  // TODO: ci pouzit vyparsovany cookie a poslat ako Bearer header return cookies.token
}

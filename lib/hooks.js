...
import { graphQlErrorFilter, graphQlErrorFilter2 } from './utils';
...
export const useErrorHandler = () => {
  ...
  return (error) => {
    ...
    if (filtered.unknownError) {
      setAppMessage({ variables: { messages: [filtered.unknownError.message], severity: 'error' } });
    }
    return filtered.knownErrors;
  };
};

export const useErrorHandler2 = () => {
  const [setAppMessage] = useMutation(SET_APP_MESSAGE);
  return (error) => {
    console.log({ m: 'useErrorHandler2', error });
